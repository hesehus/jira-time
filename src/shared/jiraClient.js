...
function startupCheck() {
    verifyLoginStatus().then(updateUserInfo);
}
...
export function updateUserInfo() {
    ...
    userInfo({ username: state.profile.username }).then(userinfo => {
...
export function getIssue({ key, url }) {
    ...
    return new Promise((resolve, reject) => {
        ...
        Promise.all([getIssue, getMeta])
            .then(([getIssueResponse, getMetaResponse]) => {
                        ...
                        response.json().then(json => {
                            ...
                            if (epicKey) {
                                callApi({
                                    ...
                                    Promise.all([getIssueResponse.json(), epicResponse.json()]).then(
                                        ([issueJson, epicJson]) => {
...
export function extractIssueKeysFromText(text) {
    const matches = text.match(/[a-zA-Z0-9]+[-][0-9]+/g);
    if (!matches) {
        return [];
    }
    return matches;
...
export function getWorkLogs({ startDate, endDate, username }) {
    ...
    return new Promise((resolve, reject) => {
        let jql =
            `worklogDate >= "${startDate.format('YYYY-MM-DD')}"` +
        ...
        callApi({
            ...
            .then(response => {
                ...
                let worklogsGetters = response.issues.map(i =>
                    getWorkLogsForIssue({ key: i.key, startDate, username })
                );
